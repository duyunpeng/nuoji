            ...
            type: function (type) {
                ...
                switch (type) {
                    ...
                    case 'integer':
                        ...
                        break;
                    case 'phoneNumber':
                        assert = new Validator.Assert().Regexp('^1[3|4|5|8][0-9]\\d{8,8}$');
                        break;
                    case "digits":
                        assert = new Validator.Assert().Regexp('^\\d+$');
                        break;
                    case 'alphanum':
                        ...
                        break;
                    case 'zhcn':
                        assert = new Validator.Assert().Regexp('[^\\u0000-\\u00FF]');
                        break;
                    case 'fax':
                        assert = new Validator.Assert().Regexp('^(02[0-5]|02[7-9])-\\d{8}$|^(85[2-3])-\\d{8}$|^(0[3-9][1-9][\\d])-\\d{7,8}$');
                        break;
                    case 'hotelLine':
                        assert = new Validator.Assert().Regexp('^(02[0-5]|02[7-9])-\\d{8}$|^(85[2-3])-\\d{8}$|^(0[3-9][1-9][\\d])-\\d{7,8}$');
                        break;
...
//      return 'undefined' === typeof fieldInstance.options.multiple ? fieldInstance.$element.after(fieldInstance._ui.$errorsWrapper) : fieldInstance.$element.parent().after(fieldInstance._ui.$errorsWrapper);
            //满足后台消息显示
            if ('undefined' === typeof fieldInstance.options.multiple) {
                var _$container = $("#" + fieldInstance._ui.errorsWrapperId);
                if ('undefined' !== typeof _$container && _$container.length) {
                    fieldInstance._ui.$errorsWrapper = _$container;
                    return _$container;
                } else {
                    return fieldInstance.$element.after(fieldInstance._ui.$errorsWrapper);
                }
            } else {
                return fieldInstance.$element.parent().after(fieldInstance._ui.$errorsWrapper)
            }
        },
...
//    this.__id__ = ParsleyUtils.hash(4);
        this.__id__ = $(field).attr("name") || $(field).attr("id");
        this.$element = $(field);
...
// Define then the messages
    window.ParsleyConfig.i18n.en = $.extend(window.ParsleyConfig.i18n.en || {}, {
        ...
        type: {
            ...
            number: "请输入正确的数字！",
            integer: "只能为数字！",
            phoneNumber: "请输入有效的11位数手机号码！",
            zhcn: "只能为中文！",
            fax: "请输入正确的传真号！例如：023-89812373 ",
            hotelLine: "请输入正确的座机号码！例如：023-89812373 ",
            alphanum: "请输入字母或数字！"
        },
        notblank: "请输入值！",
        required: "不能为空，请填写！",
        ...
        mincheck: "字符长度应该在 %s 到 %s 之间！",
        maxcheck: "请至少选择 %s 个选项！",
        check: "请选择不超过 %s 个选项！",
        equalto: "输入值不同！"
    });
